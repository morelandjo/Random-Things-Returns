...
public class GoldenCompassItem extends Item {
    ...
    public static void setTarget(ItemStack compass, int x, int z) {
        compass.set(ModDataComponents.COMPASS_TARGET_X.get(), x);
        compass.set(ModDataComponents.COMPASS_TARGET_Z.get(), z);
    }

    public static BlockPos getTarget(ItemStack compass) {
        Integer x = compass.get(ModDataComponents.COMPASS_TARGET_X.get());
        Integer z = compass.get(ModDataComponents.COMPASS_TARGET_Z.get());
        if (x != null && z != null) {
